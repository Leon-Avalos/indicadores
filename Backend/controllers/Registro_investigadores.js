...
let validarInvestigador = (investigador) => {
  ...
  if (!investigador.last_name) {
    throw { ok: false, mensaje: "El apellido del investigador es obligatorio.",
    };
  }
  ...
  if (!investigador.password) {
    throw { ok: false, mensaje: "La contraseña para el investigador es obligatoria.",
    };
  }
};
...
let eliminarInvestigador = async ({ researcher_document }) => {
  let _servicio = new ServicioPg();
  let sql = `DELETE FROM public.researcher WHERE  researcher_document=${researcher_document} cascade`;
